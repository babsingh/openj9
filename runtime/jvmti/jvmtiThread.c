...
resumeThread(J9VMThread * currentThread, jthread thread)
{
	...
	if (rc == JVMTI_ERROR_NONE) {
		...
		if (NULL != thread) {
			isVirtual = IS_VIRTUAL_THREAD(currentThread, J9_JNI_UNWRAP_REFERENCE(thread));
		}
		if (isVirtual) {
			/* targetThread is NULL only for an unmounted virtual threads. */
	...
	const jthread *except_list)
{
	...
	if (JVMTI_ERROR_NONE == rc) {
		jint i = 0;
		UDATA currentThreadEverSuspended = FALSE;
		j9object_t root = NULL;
		j9object_t walkVirtualThread = NULL;
		...
		printf("walking suspendall vthread list\n");
		/* Walk all virtual threads. */
//		omrthread_monitor_enter(vm->liveVirtualThreadListMutex);
		if (NULL != vm->liveVirtualThreadList) {
			root = *(vm->liveVirtualThreadList);
			printf("suspendallvthreads root %p\n", root);
			/* Skip the root, which is a dummy virtual thread and global ref. */
			walkVirtualThread = J9OBJECT_OBJECT_LOAD(currentThread, root, vm->virtualThreadLinkNextOffset);
			do {
				BOOLEAN skipSuspend = FALSE;
				for (i = 0; i < except_count; ++i) {
				...
				if (!skipSuspend) {
					UDATA currentThreadSuspended = 0;
					printf("enter suspendallvthreads walk virtual thread %p\n", walkVirtualThread);
					...
					printf("exit suspendallvthreads walk virtual thread %p\n", walkVirtualThread);
					currentThreadEverSuspended |= currentThreadSuspended;
				}
...
//		omrthread_monitor_exit(vm->liveVirtualThreadListMutex);
		...
		printf("finished suspendall vthread list\n");

		/* If the current thread appeared in the list (and was marked as suspended), block now until the thread is resumed. */
		if (currentThreadEverSuspended) {
			printf("inside suspendallvthreads currenthread suspended\n");
			vm->internalVMFunctions->internalExitVMToJNI(currentThread);
			setHaltFlag(currentThread, J9_PUBLIC_FLAGS_HALT_THREAD_JAVA_SUSPEND);
			vm->internalVMFunctions->internalEnterVMFromJNI(currentThread);
		}
	...
	const jthread *except_list)
{
	...
	if (rc == JVMTI_ERROR_NONE) {
		...
		printf("walking resumeall vthread list\n");
		/* Walk all virtual threads. */
//		omrthread_monitor_enter(vm->liveVirtualThreadListMutex);
		if (NULL != vm->liveVirtualThreadList) {
			root = *(vm->liveVirtualThreadList);
...
//		omrthread_monitor_exit(vm->liveVirtualThreadListMutex);
		printf("finished resumeall vthread list\n");
done:
		vm->internalVMFunctions->internalExitVMToJNI(currentThread);
	}
