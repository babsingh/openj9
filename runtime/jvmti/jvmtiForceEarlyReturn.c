	...
	if (rc == JVMTI_ERROR_NONE) {
		...
		if (rc == JVMTI_ERROR_NONE) {
			...
			j9object_t threadObject = (NULL == thread) ? currentThread->threadObject : J9_JNI_UNWRAP_REFERENCE(thread);
                        ...
                        printf("targetThread = %p, currentThread = %p\n", targetThread, currentThread);
                        printf("threadObject = %p, suspended = %u\n", threadObject, J9OBJECT_U32_LOAD(currentThread, threadObject, vm->isSuspendedInternalOffset));
